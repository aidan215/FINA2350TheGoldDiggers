...
# Abs volatility bucketing validation
vol_col = "Actual_Volatility"
...
for model in models:
    results[model] = results[model].merge(y[["Date", "Actual_Volatility", "bucket"]], on="Date", how="left")
    ...
    results[model]["mse"] = mean_squared_error(results[model]["Volatility_Market_Adjusted"], results[model]['Actual_Volatility'])
    
    results[model]["mae"] = mean_absolute_error(results[model]["Volatility_Market_Adjusted"], results[model]['Actual_Volatility'])
...
out_path = "results/classification_output_daily.txt"
